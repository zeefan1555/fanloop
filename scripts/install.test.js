...
function installedAt(version) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "commonloop-installed-"));
...
describe("Commonloop npm installer", () => {
  ...
  it("computes the archive SHA-256 used by release.json", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "commonloop-checksum-"));
    const file = path.join(directory, "asset.tar.xz");
    fs.writeFileSync(file, "commonloop\n");
    assert.equal(checksum(file), "sha256:2c0f356afece181dd825ea64cbdeba4a028a1eb242478cea9e85e58604b04f1d");
  });

  it("uses an explicit data root without reading a project directory", () => {
    assert.equal(dataHome({ COMMONLOOP_DATA_HOME: "/tmp/commonloop-data" }), "/tmp/commonloop-data");
  });

  it("stores matched releases under ~/.commonloop by default", () => {
    assert.equal(dataHome({}), path.join(os.homedir(), ".commonloop"));
  });

  it("keeps a newer installed Release when update resolves an older candidate", () => {
    const root = installedAt("0.1.66");
    const env = { COMMONLOOP_DATA_HOME: root, COMMONLOOP_UPDATE_FORWARD_ONLY: "1" };
  ...
  it("reports a kept Release apart from a fresh install", () => {
    assert.equal(describeInstall({ version: "0.1.66", kept: true }), "Commonloop 0.1.66 is already up to date");
    assert.equal(describeInstall({ version: "0.1.66", kept: false }), "Commonloop 0.1.66 installed successfully");
  });

  it("installs an older Release when the caller is not update", () => {
    const root = installedAt("0.1.66");
    assert.equal(keepsInstalledRelease({ COMMONLOOP_DATA_HOME: root }, "0.1.65"), "");
  });

  it("installs when either side is missing or not stable semver", () => {
    const missing = fs.mkdtempSync(path.join(os.tmpdir(), "commonloop-absent-"));
    const forwardOnly = { COMMONLOOP_UPDATE_FORWARD_ONLY: "1" };
    assert.equal(keepsInstalledRelease({ ...forwardOnly, COMMONLOOP_DATA_HOME: missing }, "0.1.65"), "");
    assert.equal(keepsInstalledRelease({ ...forwardOnly, COMMONLOOP_DATA_HOME: installedAt("dev") }, "0.1.65"), "");
    assert.equal(keepsInstalledRelease({ ...forwardOnly, COMMONLOOP_DATA_HOME: installedAt("0.1.66") }, "0.2.0-rc.1"), "");
  });
  ...
  it("rejects links before extracting a Release archive", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "commonloop-archive-"));
    const source = path.join(directory, "source");
    fs.mkdirSync(path.join(source, "bin"), { recursive: true });
    fs.writeFileSync(path.join(source, "target"), "not a binary\n");
    fs.symlinkSync("../target", path.join(source, "bin", "commonloop"));
  ...
  it("rejects an archive path outside the staging directory", () => {
    ...
    assert.doesNotThrow(() => validateArchivePath("entrypoints/commonloop-workflow/SKILL.md"));
  });
});
