...
description: 所有研发需求的默认入口。适用于收到 PRD、新研发需求、缺陷修复或代码变更请求，以及启动或继续 Workflow、执行当前 Prompt、上报 Progress 或 Condition Result 和选择回流目标。
---
...
2. Status 返回 `NOT_INITIALIZED` 且用户要启动新流程时，完整读取并执行 [`ref/role.md`](ref/role.md)。使用 `fanloop-workflow-selector` 按显式选择或默认规则得到 Workflow ID，随后运行 `flow init`；同一次新流程启动只执行一次选择。
