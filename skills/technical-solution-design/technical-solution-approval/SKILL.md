...
- 批准当前完整方案：`technical_solution_approved`，流程结束；
- 问题和方向不变，只需修改正文、架构图或实现细节：`technical_solution_rejected`；
- 核心模型、选型或总体架构方向变化：`solution_direction_changed`；
- 问题、目标、非目标或硬约束变化：`technical_problem_changed`；
- 含糊或尚需讨论：继续等待。

Evidence 保存人的完整原始回复，以及本轮审核的五个文件路径。
