...
以下逻辑产物均写入 `.technical-solution/sections/02-problem.md`：

| 逻辑产物 | 完整性要求 |
| --- | --- |
| 核心问题 | 每个独立问题有具体判断、事实差距及与已确认背景的对应关系。 |
| 根因与影响 | 用证据解释差距形成的机制及业务影响，区分现象和原因；证据缺口有验证办法。 |
| 关键矛盾 | 说明真实存在的指标冲突及条件；没有冲突时给出事实依据，不制造取舍。 |

逐项回读产物内容，核对证据与因果链。片段不得出现 `#` 或 `##`；允许按
真实内容使用 `###`，但禁止 `####` 及手工 `1.1` 式标题。使用加粗的问题结论、证据表和因果链
表达；不得写目标值、候选方案或解决手段。

若调查改变了背景语义，停止并上报 `background_changed`，Evidence 写明变化、保留内容、失效产物
和回流 Step。否则仅在产物列表逐项完整时上报 `core_problem_defined` 和片段路径。
