...
读取[范文与标准](references/exemplars.md)，按导读查看完整正文，按其中的对照表记录范文与当前文档的具体差距。
范文图片是源码仓库中的可选视觉资料，不随 CLI Release 安装；普通审校依据正文、导读和图片说明完成。
只有任务明确要求视觉对照时才查看源码中的图片；图片不可用时报告该项范围，不阻塞其余审校，
也不把材料故障当作当前方案的内容反馈。
