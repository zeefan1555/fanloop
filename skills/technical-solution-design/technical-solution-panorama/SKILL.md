...
description: 在 technical-solution-design 的 panorama_card_published Condition 要求时生成并通过当前 Agent 通道展示自包含 Panorama，返回本次真实投递回执。
---
...
2. 以 `data.content` 为流程状态骨架，按当前 Human Step Prompt 读取有效 Outputs 指向的审核产物，把要求审核的问题、方案、约束、验证与待决事项合并为一份无需聊天历史和外部链接也能理解的材料。不得把原始渲染结果或链接清单冒充完整审核材料。
