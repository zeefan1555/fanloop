...
description: 执行子 Agent编排 Fanloop CLI 需求澄清，把已对齐目标、公开 CLI 验收场景和主 Agent决定沉淀到 requirements.md 与唯一飞书需求文档。
---
...
4. 执行子 Agent展示完整计划并向主 Agent申请批准或拒绝；不得自批，也不直接向用户索取内部流程确认。只有用户主动改变目标、范围或验收标准时才由主 Agent重新对齐。
5. 收到明确回复后，使用 `fanloop-dev-decision-receipt` 记录主 Agent actor、当前 turn、原始决定和证据，并把 receipt ID 与决定写回 `requirements.md`。
