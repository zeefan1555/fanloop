...
- Verification：baseline/candidate 必须是同一 Case；local-test-report.md 的 profile、candidate HEAD、
  命令、退出码和测试前后源码状态必须覆盖当前 reviewed HEAD。Feature Map、验证 Skill 或测试资产变化
  会使旧报告失效。

真实 Release 安装、逐 Feature live 维护和机器人 Eval 属于 Review 之后，不以其缺失阻断本 Step。

通过或回流都先写 review-report.md，包含 reviewed HEAD、main...HEAD、findings、结论和 ADR impact：

- 无阻断项：review_passed + review_report_written。
