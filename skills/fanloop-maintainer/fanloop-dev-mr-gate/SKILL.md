...
2. `origin/main == review_base` 时上报 `handoff_main_unchanged`。若只有 main 前进，在当前 Step 合入精确 SHA；冲突使用 `resolving-merge-conflicts`。合入后对 `reviewed_head..final_head` 和 `final_base...final_head` 做独立 CR，运行受影响聚焦测试、`./tests/run-unit`、`./tests/run-e2e`，写入 `handoff-integration-report.md`；只在 human 明确确认未改变原批准需求后上报集成四项 Condition。
