---
name: commonloop-dev-mr-handoff
description: 在本地验证和代码审查通过后，幂等 push Commonloop CLI 分支、创建或更新 GitHub PR，并用 Botmux 发起人工审核话题。
---
...
1. 读取当前分支与 HEAD，要求 `origin` 精确指向 `zeefan1555/commonloop` GitHub 仓库；未配置远端时
   停止并报告，不自行创建仓库或添加远端。用
   `gh pr list --repo zeefan1555/commonloop --state open --head <branch> --json number,url,baseRefName,headRefName`
   查询。
2. 先用 `git push -u origin HEAD` 幂等推送。唯一命中时用
   `gh pr edit <number> --repo zeefan1555/commonloop --title <title> --body-file <body-file>` 更新；
   无命中时用
   `gh pr create --repo zeefan1555/commonloop --base main --head <branch> --title <title> --body-file <body-file>`
