---
name: commonloop-dev-workflow
description: 维护 zeefan1555/commonloop 自身的入口。用于自我迭代机器人接收缺陷、优化或代码变更请求，并沿 commonloop-maintainer Workflow 从需求澄清推进到最终 MR 交接。
---

# Commonloop Dev Workflow

维护 `zeefan1555/commonloop` 时，以 CLI 返回为唯一流程依据，持续执行：

**读取 Status -> 执行 Prompt/Skills -> 上报 Progress 或 Result -> 读取新 Status**

`.commonloop` 由 CLI 管理。构造命令输入前读取目标叶子命令的 `--help`。新 Requirement
使用 `~/commonloop/issues/<issue-slug>`；已有 Requirement 使用包含 `.commonloop` 的目录。
...
`~/commonloop/issues/*/.commonloop/card/config.json` 并精确比较 `session_id`：
...
收到 Commonloop CLI 的缺陷、优化或代码变更请求默认表示启动新维护流程；只有用户明确要求仅
回答且不进入研发流程时才跳过。

1. Status 返回已初始化 State 时，直接继续其固定的 `commonloop-maintainer` Workflow，不更新 Release
   或重新选择 Workflow。
2. 仅当 Status 返回 `NOT_INITIALIZED` 且用户要启动新流程时，完整读取并执行
   [`ref/role.md`](ref/role.md)，再使用当前配套 Release 的固定 Workflow ID 初始化：

   ```bash
   commonloop flow init --root <ABSOLUTE_ROOT> --workflow commonloop-maintainer ...
   ```
...
4. 工作尚未形成退出结论时上报：
   ...
   ```bash
   commonloop flow report progress --step-id <当前 Step ID> --status <in_progress|fixing|blocked> --summary <摘要> [--evidence '<JSON>']
   ```
...
5. 形成真实结论后，从 `current.conditions[]` 选择原子 Condition，并按其 `output.type` 与约束
   ...
   ```bash
   commonloop flow report result --step-id <当前 Step ID> --condition-result '{"condition_id":"<ID>","output":{"type":"<TYPE>","value":<JSON>}}' [--condition-result ...] <--next-step-id ID|--back-step-id ID|--terminal> --summary <摘要> [--evidence '<JSON>']
   ```
...
进入 Human Step 前，在 `requirements.md` 记录 `expectedApprover`：当前 `commonloop-dev` 应用
...
Panorama 审核卡发送成功后，需要实现时，正文去除首尾空白后必须精确等于 `批准进入 需求实现`。
`同意`、`按这个改`、方案反馈、
