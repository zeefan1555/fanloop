...
status: accepted
date: 2026-09-17
amends: ADR-0092, ADR-0097, ADR-0100
---

# Maintainer 合并 PR 并更新本地 Fanloop

`fanloop-maintainer` 保持 ADR-0100 的前三个 Stage、前三个 Job 和前八个 Step 不变，只把第九个
Step 从：
...
PR。只有 PR 为 `MERGED`、merge commit 非空、第一 parent 等于 `final_base` 且可达 `origin/main` 才视为
合码完成。
...
`delivery-record.md` 存在时必须一致，缺失时从远端已合并事实重建。允许源码 worktree 已位于该 detached
commit，并只重试未完成阶段。安装前清除 data、config、Skill Root 和 Botmux 环境覆盖。PR 描述保留
背景、问题、解法、影响、验证、ADR impact、human 结论和交付边界。
...
生产五文件变为 53 Conditions、38 个 Flow Route objects、45 个 Loop Route objects 和 58 Prompts。
