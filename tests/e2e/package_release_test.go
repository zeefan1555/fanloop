...
func TestPackagedNPMInstallerUsesItsMatchedReleaseManifest(t *testing.T) {
	...
	for _, target := range []struct{ os, arch string }{
		...
		linker := strings.Join([]string{
			"-s",
			"-w",
			"-X github.com/zeefan1555/commonloop/internal/buildinfo.ReleaseVersion=1.2.3",
			"-X github.com/zeefan1555/commonloop/internal/buildinfo.CLIVersion=1.2.3",
			"-X github.com/zeefan1555/commonloop/internal/buildinfo.Commit=package-test",
		}, " ")
	...
	if readme, err := os.ReadFile(filepath.Join(packageRoot, "README.md")); err != nil || !bytes.Contains(readme, []byte("~/.commonloop/current")) || !bytes.Contains(readme, []byte("commonloop-workflow")) {
	...
	install := exec.Command("npx", "--yes", "--package", artifact, "commonloop", "install")
	...
	if installErr != nil {
		...
		diagnostic := exec.Command(filepath.Join(diagnosticRoot, "bin", "commonloop"), "doctor")
	...
	for _, root := range []string{traeRoot, claudeRoot} {
		...
		if err != nil || len(installedLinks) != 1 || installedLinks[0].Name() != "commonloop-workflow" {
			t.Fatalf("installed Skill links in %s = %#v, want only commonloop-workflow: %v", root, installedLinks, err)
		}
	}
	if err := os.RemoveAll(npmCache); err != nil {
		t.Fatal(err)
	}
	launcher := filepath.Join(npmPrefix, "bin", "commonloop")
