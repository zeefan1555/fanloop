...
2. Fanloop maintainer Workflow 的本地验证与代码审查只使用当前工作树；审查通过后冻结 `candidate_head`。真实 Agent 验收必须从同一候选安装隔离 Release，并由一个无实现上下文的全新 Sub-agent 使用公开 CLI 验证需求中批准的场景；不得拿已发布版本、旧安装、机器人或源码阅读当证据。随后发布唯一 PR，CI 硬门禁回读该精确 SHA 的 Ruleset 与 required checks，不能替代或倒推本地验证。源码、测试或验证资产更新后，旧本地验证、Review、Sub-agent 验收与 CI 全部失效，必须形成新 HEAD 并从本地验证重跑。
...
完成标准：聚焦命令通过，本地验证与 AI Code Review 覆盖冻结的最终候选 HEAD；`e2e` profile 还要求本地 `run-unit` 与 `run-e2e` 零退出，报告对应当前工作树和二进制，且测试执行未改变源码状态。隔离候选的 Sub-agent 黑盒验收通过后发布 PR；远端 Ruleset 与 required checks 在同一 `candidate_head` 上通过才可自动合码。合并后还必须把精确 merge commit 从干净 detached worktree 安装为本地 current，并回读 version commit 与 Doctor。
