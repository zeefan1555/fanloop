...
func skillCheck(root string, manifest release.Manifest) *idl.DoctorCheck {
	for _, skill := range manifest.Skills {
		...
		if err != nil {
			return check("skills", statusFail, err.Error(), "Reinstall this Fanloop release.")
		}
...
func skillLinkCheck(releaseRoot string, roots []string, manifest release.Manifest) *idl.DoctorCheck {
	...
	for _, root := range roots {
		...
		if err != nil || info.Mode()&os.ModeSymlink == 0 {
			return check("skill_links", statusFail, fmt.Sprintf("Skill link %q is missing or is not managed by Fanloop.", path), "Reinstall Fanloop without replacing user-owned paths.")
		}
		...
		if actualErr != nil || filepath.Clean(actual) != filepath.Clean(expected) {
			return check("skill_links", statusFail, fmt.Sprintf("Skill link %q does not point to the current release.", path), "Reinstall Fanloop to restore the managed Skill links.")
		}
	}
...
func workflowCheck(root string, manifest release.Manifest) *idl.DoctorCheck {
	for _, item := range manifest.Workflows {
		...
		if err != nil {
			return check("workflows", statusFail, err.Error(), "Reinstall this Fanloop release.")
		}
		loaded, err := workflow.LoadDirectory(path)
		if err != nil || loaded.Ref.Digest != item.Sha256 {
			return check("workflows", statusFail, fmt.Sprintf("Workflow %q checksum does not match.", item.Id), "Reinstall this Fanloop release.")
		}
		if loaded.Ref.ID != item.Id {
			return check("workflows", statusFail, fmt.Sprintf("Workflow %q is invalid.", item.Id), "Reinstall this Fanloop release.")
		}
	}
...
func versionCheck(manifest release.Manifest) *idl.DoctorCheck {
	if manifest.ReleaseVersion != buildinfo.ReleaseVersion || manifest.Cli.Version != buildinfo.CLIVersion {
		return check("version_drift", statusFail, "Release manifest and running CLI versions differ.", "Switch to one complete Fanloop release.")
	}
	...
	if err != nil || current.StateSchema.WriteVersion != manifest.StateSchema.WriteVersion || !equalInts(current.StateSchema.ReadVersions, manifest.StateSchema.ReadVersions) {
		return check("version_drift", statusFail, "State Schema support differs from the release manifest.", "Switch to one complete Fanloop release.")
	}
	actual := map[string]string{}
	embedded, listErr := workflow.List()
	if listErr != nil {
		return check("version_drift", statusFail, "Embedded Workflows cannot be read.", "Switch to one complete Fanloop release.")
	}
	for _, item := range embedded {
		actual[item.Ref.ID] = item.Ref.Digest
	}
	if len(actual) != len(manifest.Workflows) {
		return check("version_drift", statusFail, "Embedded Workflows differ from the release manifest.", "Switch to one complete Fanloop release.")
	}
	for _, item := range manifest.Workflows {
...
func requirementChecks(root string) []*idl.DoctorCheck {
	...
	stateContent, readErr := os.ReadFile(filepath.Join(root, ".fanloop", "flow", "state.json"))
	...
	if jsonErr != nil || loose.SchemaVersion != state.CurrentStateSchemaVersion {
		checks = append(checks, check("state_schema", statusFail, "State JSON or schema version is invalid.", "Repair or restore .fanloop/flow/state.json."))
	} else {
	...
	if strictErr != nil {
		checks = append(checks, check("state_invariants", statusFail, "State invariants are invalid: "+strictErr.Error(), "Repair or restore .fanloop/flow/state.json."))
	} else if !bindingOK {
	...
	if outputsErr != nil {
		checks = append(checks, check("outputs", statusFail, "Output Registry is invalid: "+outputsErr.Error(), "Repair or restore .fanloop/output/state.json."))
	...
	if eventsErr != nil {
		...
		actual, err := os.ReadFile(filepath.Join(root, ".fanloop", "trace", "events.md"))
		if err != nil || !bytes.Equal(actual, expected) {
			checks = append(checks, check("trace_projection", statusWarn, "Events Markdown is missing or stale.", "Run fanloop trace render --root <requirement>."))
		} else {
...
func validateCardSnapshots(root string) error {
	directory := filepath.Join(root, ".fanloop", "card")
	...
	for _, entry := range entries {
		...
		relative := ".fanloop/card/" + entry.Name()
...
func validateCardProjection(root string, current state.State, hasCurrent bool) *idl.DoctorCheck {
	...
	if err != nil {
		return check("card_projection", statusFail, "Card projection is invalid: "+err.Error(), "Repair or restore .fanloop/card/projection.json.")
	}
	if hasCurrent {
		...
		if projected.Requirement != current.Requirement || projected.Release != current.Release ||
			...
			return check("card_projection", statusFail, "Card projection is stale.", "Run an accepted Flow report to refresh .fanloop/card/projection.json.")
		}
	}
...
func validateCardBinding(root string) *idl.DoctorCheck {
	...
	if err != nil {
		return check("card_binding", statusFail, "Card binding config is invalid: "+err.Error(), "Repair .fanloop/card/config.json and keep its permissions at 0600.")
	}
	if !configured {
