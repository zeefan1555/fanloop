...
func TestApprovedPromotionDesignBundleLoads(t *testing.T) {
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate repository")
	}
	review := filepath.Clean(filepath.Join(filepath.Dir(file), "..", "..", "workflows", "promotion-design"))

	loaded, err := LoadDirectory(review)
	if err != nil {
		t.Fatal(err)
	}
	if loaded.Ref.ID != "promotion-design" || loaded.Ref.Digest == "" {
